...
PACKAGE BODY IPXSockets IS
   ...
   IPXSocketsClassData : classData;
   FOR IPXSocketsClassData USE AT standard.system.label("IPXSocketsClassData");
   ...
   IPXSocketsCClassData : CClassData;
   FOR IPXSocketsCClassData USE AT standard.system.label("IPXSocketsCClassData");
   ...
   BEGIN
      IF 
         SOMObject.somIsA(
            ...
            IPXSocketsClassData.classObject) 
      THEN
   ...
   FUNCTION New_Class (
      ...
      RETURN             SOM.SOMClassPtr
   IS
      FUNCTION IPXSocketsNewClass (
         Major_Version : IN SOM.integer4;
         Minor_Version : IN SOM.integer4)
         RETURN             SOM.SOMClassPtr;
      PRAGMA interface (C, IPXSocketsNewClass);
      PRAGMA interface_information (IPXSocketsNewClass, ".IPXSocketsNewClass");
   BEGIN
      RETURN IPXSocketsNewClass(major_version, minor_version);
   END New_Class;
   ...
   BEGIN
      IF IPXSocketsClassData.classObject = SOM.null_address THEN
         Dummy := New_Class(Major_Version, Minor_Version);
      END IF;
      Result.som_object := 
         SOMClass.somNew(
            SOMClass.To_Ref(IPXSocketsClassData.classObject));
      RETURN Result;
   END New_Instance;
